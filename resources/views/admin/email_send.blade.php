							...
							<form method="post" action="/<?= admin ?>/multiEmail" enctype="multipart/form-data">
								...
								<div class="col-md-9">
									<div class="row">
										<br>
										@php
										$row = DB::table('contactusers')->where('id', '=', request('id'))->first();
										@endphp
										@if (request("id"))
										...
										<div class="col-md-6">
											<div class="form-group">
												...
												<input type="number" name="id_from" id="title" class="form-control" min="{{ (!empty($first_id))?$first_id:0 }}" max="{{ (!empty($last_id))?$last_id:0 }}" value="{{ (!empty($first_id))?$first_id:0 }}" required="">
											</div>
										</div>
										<div class="col-md-6">
											<div class="form-group">
												<label class="req">Select id to</label>
												<input type="number" name="id_to" id="title" class="form-control" min="{{ (!empty($first_id))?$first_id:0 }}" max="{{ (!empty($last_id))?$last_id:0 }}" value="{{ (!empty($last_id))?$last_id:0 }}" required="">
											</div>
										</div>
										@endif
									</div>
									...
									<div class="form-group">
										...
										<textarea name="email_content" class="form-control oneditor" id="oneditor" rows="25"  aria-hidden="true"></textarea>
										<div class="text-danger"></div>
									</div>
									<div style="padding:10px;">
										<button type="submit" name="submit" value="send" class="btn btn-danger">Send Email</button>
									</div>
								</div>
							</form>
						</div>
